...
class GetHeadersMessageTests: XCTestCase {
  ...
  let getHeadersMessageWithHashStopBytes: [UInt8] = [
  ...
  let getHeadersMessageWithoutHashStopBytes: [UInt8] = [
      0x71, 0x11, 0x01, 0x00,                           // 70001 (protocol version 70001)
      0x02,                                             // Number of block locator hashes (2)
      0x0b, 0x73, 0x25, 0x7c, 0x50, 0x9b, 0xf0, 0xa1,
      0xc6, 0xc4, 0x72, 0xbc, 0xae, 0xa9, 0x64, 0x26,
  ...
  var getHeadersMessageWithHashStopData: NSData!
  var getHeadersMessageWithoutHashStopData: NSData!

  var getHeadersMessageWithHashStop: GetHeadersMessage!
  ...
  override func setUp() {
    ...
    getHeadersMessageWithHashStop =
        GetHeadersMessage(protocolVersion: 70001,
                          blockLocatorHashes: [blockLocatorHash0, blockLocatorHash1],
                          blockHashStop: blockHashStop)
    getHeadersMessageWithoutHashStop =
        GetHeadersMessage(protocolVersion: 70001,
                          blockLocatorHashes: [blockLocatorHash0, blockLocatorHash1])
    getHeadersMessageWithHashStopData = NSData(bytes: getHeadersMessageWithHashStopBytes,
                                              length: getHeadersMessageWithHashStopBytes.count)
    getHeadersMessageWithoutHashStopData =
        NSData(bytes: getHeadersMessageWithoutHashStopBytes,
               length: getHeadersMessageWithoutHashStopBytes.count)
  }

  func testGetHeadersMessageEncodingWithHashStop() {
    XCTAssertEqual(getHeadersMessageWithHashStop.bitcoinData, getHeadersMessageWithHashStopData)
  }

  func testGetHeadersMessageEncodingWithoutHashStop() {
    XCTAssertEqual(getHeadersMessageWithoutHashStop.bitcoinData,
                   getHeadersMessageWithoutHashStopData)
  }

  func testGetHeadersMessageDecodingWithHashStop() {
    let stream = NSInputStream(data: getHeadersMessageWithHashStopData)
    stream.open()
    if let testGetHeadersMessage = GetHeadersMessage.fromBitcoinStream(stream) {
      XCTAssertEqual(testGetHeadersMessage, getHeadersMessageWithHashStop)
    } else {
  ...
  func testGetHeadersMessageDecodingWithoutHashStop() {
    let stream = NSInputStream(data: getHeadersMessageWithoutHashStopData)
    stream.open()
    if let testGetHeadersMessage = GetHeadersMessage.fromBitcoinStream(stream) {
      XCTAssertEqual(testGetHeadersMessage, getHeadersMessageWithoutHashStop)
    } else {
      XCTFail("Failed to parse GetHeadersMessage")
    }
    XCTAssertFalse(stream.hasBytesAvailable)
