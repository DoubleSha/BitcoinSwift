...
class GetHeadersMessageTests: XCTestCase {
  ...
  func testGetHeadersMessageEncoding() {
    ...
    XCTAssertEqual(getHeadersMessage.bitcoinData, expectedData)
  }
  ...
  func testGetHeadersMessageDecoding() {
    ...
    stream.open()
    if let getHeadersMessage = GetHeadersMessage.fromBitcoinStream(stream) {
  ...
  func testGetHeadersMessageWithNoBlockHashStopDecoding() {
    let bytes: [UInt8] = [
        0x71, 0x11, 0x01, 0x00,                           // 70001 (protocol version 70001)
        0x02,                                             // Number of block locator hashes (2)
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
