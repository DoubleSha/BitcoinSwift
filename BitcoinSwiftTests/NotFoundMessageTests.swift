...
class NotFoundMessageTests: XCTestCase {
  ...
  func testNotFoundMessageDecoding() {
    ...
    if let NotFoundMessage = NotFoundMessage.fromData(data) {
      XCTAssertEqual(NotFoundMessage.inventoryVectors.count, 1)
      ...
      XCTAssertEqual(NotFoundMessage.inventoryVectors, expectedInventoryVectors)
    } else {
  ...
  func testNotFoundMessageEncoding() {
    ...
    let inventoryVectors = [
        InventoryVector(type:InventoryVector.VectorType.Block,
                        hash:NSData(bytes:vector1Hash, length:vector1Hash.count))]
    let NotFoundMessage = InventoryMessage(inventoryVectors:inventoryVectors)
    ...
    XCTAssertEqual(NotFoundMessage.data, expectedData)
  }
}
