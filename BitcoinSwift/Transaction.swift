...
public struct Transaction: MessagePayload, Equatable {
  ...
  public static func fromData(data: NSData) -> Transaction? {
    ...
    if version == nil {
      println("WARN: Failed to parse version in Transaction \(data)")
      return nil
    }
    let inputCount = stream.readVarInt()
    if inputCount == nil {
      println("WARN: Failed to parse inputCount in Transaction \(data)")
      return nil
    }
    var inputs: [Input] = []
    for i in 0..<inputCount! {
      ...
      if input == nil {
        println("WARN: Failed to parse input at index \(i) in Transaction \(data)")
        return nil
      }
      inputs.append(input!)
    }
    ...
    if outputCount == nil {
      println("WARN: Failed to parse outputCount in Transaction \(data)")
      return nil
    }
    var outputs: [Output] = []
    for i in 0..<outputCount! {
      ...
      if output == nil {
        println("WARN: Failed to parse output at index \(i) in Transaction \(data)")
        return nil
      }
    ...
    if lockTimeRaw == nil {
      println("WARN: Failed to parse lockTime in Transaction \(data)")
      return nil
    }
